    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        dashboardViewModel.getPokemon(id).observe(viewLifecycleOwner, Observer { pokemonValue ->
            pokemonValue?.let { pokemon ->
                ...
                pokemon.height.let { height ->
                    heightLabel.text =
                        PokemonStringUtil().formatPounds(
                            Converters().fromCentimetersToFeet(height))
                }

                pokemon.weight.let { weight ->
                    weightLabel.text =
                        PokemonStringUtil().formatFeet(
                            Converters().fromKilogramsToPounds(weight))
                }
        ...
        dashboardViewModel.getSpecies(id).observe(viewLifecycleOwner, Observer { speciesValue ->
            speciesValue?.let { species ->
                ...
                descriptionField.text = species.flavorTextEntries.find {
                    it.language.name == ENGLISH
                }?.flavorText?.replace("\n", " ")

                genderField.text =
                    PokemonStringUtil().formatGenterRate(species.genderRate)

                eggCycleField.text =
                    species.hatchCounter.toString()

                eggGroupsField.text =
                    species.eggGroups.map { it.name.capitalize() }.toString()
